...
vector<int> afAdapter::getGPUData(vector<int> cpudata) {
    ...
    std::cout << "Time taken for copying from CPU to GPU is " <<std::accumulate(durations.begin(),
                                                    durations.end(), 0) / durations.size() << " microseconds" << std::endl;
...
vector<int> afAdapter::getCPUData(vector<int> data) {
    ...
    std::cout << "Time taken for copying from GPU to CPU is " <<std::accumulate(durations.begin(),
                                                                                durations.end(), 0) / durations.size() << " microseconds" << std::endl;
